    ...
    <main className="min-h-[80vh] bg-white px-4 py-10 text-slate-950 sm:px-6 lg:px-8">
      <section className="mx-auto grid max-w-6xl gap-10 lg:grid-cols-[minmax(0,0.95fr)_minmax(360px,0.75fr)]">
        <div className="border border-slate-200 bg-white p-6 sm:p-8 lg:p-10">
          <span className="font-sans text-[11px] font-extrabold uppercase tracking-[0.2em] text-red-600">
            Partner With Us
          </span>
          <h1 className="mt-4 font-serif text-4xl font-extrabold leading-tight text-slate-950 md:text-5xl">
            {page.title}
          </h1>
          <p className="mt-5 max-w-3xl font-sans text-lg leading-8 text-slate-700">
            {page.intro}
          </p>
          <div className="mt-8 border-l-4 border-black bg-slate-50 p-6">
            <h2 className="font-serif text-2xl font-bold leading-tight text-slate-950">
              How New Times can help
            </h2>
            <p className="mt-3 font-sans text-base leading-7 text-slate-600">
              {page.details}
            </p>
          </div>
          <div className="mt-8 flex flex-wrap gap-3">
            {Object.values(partnerPages).map((item) => (
              <Link
                ...
                className={`border px-4 py-2 font-sans text-xs font-black uppercase tracking-[0.12em] no-underline ${
        ...
        <aside className="border border-slate-200 bg-slate-50 p-6 sm:p-8">
          <span className="font-sans text-[11px] font-extrabold uppercase tracking-[0.18em] text-slate-500">
            Contact Us
          </span>
          ...
          <form className="mt-6 space-y-4">
            <label className="block">
              <span className="font-sans text-xs font-bold uppercase tracking-[0.12em] text-slate-600">Name</span>
              <input className="mt-2 h-11 w-full border border-slate-300 bg-white px-3 font-sans text-sm outline-none focus:border-red-600" />
            </label>
            <label className="block">
              <span className="font-sans text-xs font-bold uppercase tracking-[0.12em] text-slate-600">Email</span>
              <input type="email" className="mt-2 h-11 w-full border border-slate-300 bg-white px-3 font-sans text-sm outline-none focus:border-red-600" />
            </label>
            <label className="block">
              <span className="font-sans text-xs font-bold uppercase tracking-[0.12em] text-slate-600">Phone</span>
              <input className="mt-2 h-11 w-full border border-slate-300 bg-white px-3 font-sans text-sm outline-none focus:border-red-600" />
            </label>
            <label className="block">
              <span className="font-sans text-xs font-bold uppercase tracking-[0.12em] text-slate-600">Message</span>
              <textarea
                rows="5"
                defaultValue={`I am interested in ${page.title}.`}
                className="mt-2 w-full resize-none border border-slate-300 bg-white px-3 py-3 font-sans text-sm leading-6 outline-none focus:border-red-600"
              />
            </label>
            <button
              type="button"
              className="w-full bg-red-600 px-5 py-3 font-sans text-sm font-black uppercase tracking-[0.12em] text-white hover:bg-red-700"
            >
              Submit Enquiry
            </button>
          </form>
