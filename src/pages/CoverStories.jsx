  ...
  return (
    ...
    >
      <div className={`absolute left-1/2 top-0 -translate-x-1/2 bg-white shadow-lg ${large ? 'h-full w-[268px]' : 'h-full w-[170px]'}`}>
        <span className="absolute right-3 top-1 bg-red-600 px-1.5 py-0.5 text-[7px] font-black uppercase text-white">
          Daily Cover
        </span>
        <p className={`absolute left-1/2 top-6 -translate-x-1/2 font-black leading-none text-white [text-shadow:0_2px_2px_rgba(0,0,0,.45)] ${large ? 'text-[58px]' : 'text-[34px]'}`}>
          NewTimes
        </p>
        <div
        ...
        <p className={`absolute bottom-6 left-1/2 -translate-x-1/2 whitespace-nowrap font-normal uppercase tracking-[0.03em] text-white [text-shadow:0_2px_2px_rgba(0,0,0,.55)] ${large ? 'text-[44px]' : 'text-[22px]'}`}>
  ...
  return (
    <aside className="hidden pl-3 border-l border-slate-300 lg:block">
      ...
      <div className="mt-2 flex h-[314px] items-center justify-center overflow-hidden bg-[radial-gradient(circle_at_45%_100%,#ffffff_0_32%,transparent_33%),linear-gradient(135deg,#5b00b8_0_20%,#ffffff_21%_62%,#5b00b8_63%_100%)] px-7 text-center">
        <p className="text-[24px] font-black leading-tight text-blue-700">
      ...
      <div className="overflow-hidden bg-white border mt-7 border-slate-200">
        ...
        <div className="relative rounded-t-[24px] bg-white px-4 pb-5 pt-5">
          <div className="absolute -top-6 left-4 flex h-[74px] w-[74px] items-center justify-center rounded-full bg-black text-[22px] font-bold text-white ring-4 ring-white">
            NT
          </div>
          <div className="ml-[92px]">
            <p className="text-[25px] font-black text-black">
              New-Times <span className="text-blue-600">*</span>
            </p>
            <p className="text-[16px] text-black">575K followers</p>
          </div>
          <p className="mt-7 text-[15px] leading-snug text-black">
            New-Times - a business magazine from Network18 -...
          </p>
          <button type="button" className="mt-6 w-full rounded-[6px] bg-blue-600 py-3 text-[20px] font-bold text-white">
            Follow
          </button>
        </div>
      </div>
      ...
      <div className="mt-7 flex min-h-[560px] items-center justify-center overflow-hidden bg-[radial-gradient(circle_at_30%_0,#ffffff_0_26%,transparent_27%),linear-gradient(135deg,#5b00b8_0_18%,#ffffff_19%_100%)] px-7 text-center">
        <p className="text-[32px] font-black leading-tight text-blue-700">
          Where Market
          <br />
          Leaders Came
          <br />
          ...
          <div className="flex flex-wrap gap-8 mt-9">
            {tabs.map((tab, index) => (
              <button
                key={tab}
                type="button"
                className={`px-3 py-3 text-[18px] font-bold ${
          ...
          <div className="mt-[50px] grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {stories.map((story) => (
              <article key={story.title} className="relative">
                ...
                {story.label && (
                  <span className="absolute right-3 top-[158px] bg-black px-3 py-1 text-[15px] text-white">
                    {story.label}
                  </span>
                )}
                <h2 className="mt-2 text-[23px] font-bold leading-[1.25] text-black">
                  {story.title}
                </h2>
                <p className="mt-4 text-[16px] leading-snug text-black">By {story.author}</p>
              </article>
            ))}
          </div>
        </section>
